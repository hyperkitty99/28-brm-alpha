...
class EditorPlayState extends MusicBeatSubstate
{
	...
	public function new(playbackRate:Float)
	{
		...
		dataTxt.setFormat(Paths.font("FallingSkyBd.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		dataTxt.scrollFactor.set();
		dataTxt.borderSize = 1.25;
		add(dataTxt);

		var tipText:FlxText = new FlxText(10, FlxG.height - 24, 0, 'Press ESC to Go Back to Chart Editor', 16);
		tipText.setFormat(Paths.font("FallingSkyBd.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		DiscordClient.changePresence('Playtesting on Chart Editor', states.PlayState.SONG.song, null, true, songLength);
		#end
		RecalculateRating();
	}
	...
	{
		if (states.PlayState.SONG.needsVoices && FlxG.sound.music.time >= -ClientPrefs.data.noteOffset)
		{
	...
	{
		if (states.PlayState.SONG.notes[curSection] != null)
		{
			if (states.PlayState.SONG.notes[curSection].changeBPM)
				Conductor.bpm = states.PlayState.SONG.notes[curSection].bpm;
		}
		super.sectionHit();
	}
	...
	function generateSong(dataPath:String)
	{
		...
		songSpeed = states.PlayState.SONG.speed;
		var songSpeedType:String = ClientPrefs.getGameplaySetting('scrolltype');
		switch(songSpeedType)
		{
			case "multiplicative":
				songSpeed = states.PlayState.SONG.speed * ClientPrefs.getGameplaySetting('scrollspeed');
			case "constant":
		...
		var boyfriendVocals:String = loadCharacterFile(states.PlayState.SONG.player1).vocals_file;
		var dadVocals:String = loadCharacterFile(states.PlayState.SONG.player2).vocals_file;
		...
		unspawnNotes.sort(states.PlayState.sortByTime);
	}
	
	private function generateStaticArrows(player:Int):Void
	{
		var strumLineX:Float = ClientPrefs.data.middleScroll ? states.PlayState.STRUM_X_MIDDLESCROLL : states.PlayState.STRUM_X;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		var key:Int = states.PlayState.getKeyFromEvent(keysArray, eventKey);
	...
	private function keyPressed(key:Int)
	{
		...
		plrInputNotes.sort(states.PlayState.sortHitNotes);
	...
	{
		if (states.PlayState.SONG.needsVoices && opponentVocals.length <= 0)
			vocals.volume = 1;
